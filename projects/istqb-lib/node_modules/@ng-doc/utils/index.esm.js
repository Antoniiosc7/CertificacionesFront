...
const handle = zwitch('type', {
  invalid,
  unknown,
  handlers: {comment, doctype, element: element$1, raw, root, text}
});
  ...
  return tree => visitParents(tree, 'element', (node, ancestors) => {
    ...
    if (isInlineCode || languages.includes(lang)) {
      visitParents(node, 'text', (node, ancestors) => {
        ...
        const nodes = getNodes(node, parent, isInlineCode, config);
 ...
 * @param config
 */
function getNodes(node, parent, inlineLink, config) {
  const regexp = '([*A-Za-z0-9_$@]+[.#]?[A-Za-z0-9_-]+(?:\\?.+)?)';
  const KeywordRegExp = inlineLink ? new RegExp(`^${regexp}$`, 'g') : new RegExp(regexp, 'g');
  const keywordAnchorRegexp = /^(?<keyword>[*A-Za-z0-9_$@]+)((?<delimiter>[.#])(?<anchor>[A-Za-z0-9_-]+))?(?<queryParams>(?:\?.+))?$/;
  const {
    addUsedKeyword,
    addPotentialKeyword,
    getKeyword,
  ...
  return toString(node).split(KeywordRegExp).filter(word => word.length).map(word => {
    const pureKeyword = word.split('?')[0];
    const match = word.match(keywordAnchorRegexp);
    const keyword = getKeyword(pureKeyword);
    const rootKeyword = getKeyword(match?.groups?.['keyword'] || '');
    if (keywordAnchorRegexp.test(pureKeyword)) {
      keyword ? addUsedKeyword(pureKeyword) : addPotentialKeyword(pureKeyword);
    }
    const notFoundGuideKeyword = /^\*\w+/gm.test(pureKeyword) && !keyword;
    const notFoundApiKeyword = !!rootKeyword && !!match?.groups?.['anchor'] && !keyword;
    ...
    if (inlineLink && keyword?.isCodeLink === false) {
      parent.tagName = 'a';
      parent.properties = {
        href: `${keyword.path}${match?.groups?.['queryParams'] ?? ''}`
      };
      return {
        type: 'text',
        value: keyword.title
    ...
    return keyword ? createLinkNode(inlineLink ? keyword.title : word, keyword.path, keyword.type, keyword.description) : {
      type: 'text',
      value: word
    };
  });
 ...
 * @param config
 */
async function htmlPostProcessor(html, config) {
  return unified().use(rehypeParse, {
    fragment: true
  }).use(rehypeStringify).use(keywordsPlugin, config).use(markElementsPlugin).use(markCodeBlocksPlugin).process(html).then(file => file.toString());
}
 ...
 * @param route
 */
function autolinkHeading(route) {
  return tree => visit(tree, 'element', node => {
    if (route && isElement$1(node, HEADINGS) && node.properties && node.properties['id'] && !hasClass(node, NO_ANCHOR_CLASS)) {
      node.children.push({
        type: 'element',
        tagName: 'a',
        properties: {
...
function highlightCodeLines() {
  return tree => {
    visit(tree, 'element', (node, _i, parent) => {
      if (parent?.type === 'element' && parent.tagName === 'pre' && node.tagName === 'code') {
        const highlightedLines = JSON.parse(attrValue(node, 'highlightedLines') ?? '[]');
        highlightedLines.length && node.children.forEach((child, i) => {
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
 ...
 * @param headings
 */
function sluggerPlugin(addAnchor, headings) {
  ...
  return tree => {
    ...
    visit(tree, 'element', node => {
      ...
      const dataToSlug = isHeading ? toString(node) : attrSlug;
      if (dataToSlug) {
        if (node.properties) {
 ...
 * @param config
 */
async function htmlProcessor(html, config) {
  return unified().use(rehypeParse, {
    fragment: true
  }).use(rehypeStringify).use(rehypeFormat).use(rehypeHighlight, {
    ignoreMissing: true,
    languages: {
      twig
    }
  }).use(codeBlockLinesPlugin).use(highlightCodeLines).use(sluggerPlugin, config.addAnchor, config.headings).use(rehypeMinifyWhitespace).use(autolinkHeading, config.route).process(html).then(file => file.toString());
}
...
export { htmlPostProcessor, htmlProcessor, removeNotIndexableContent };
