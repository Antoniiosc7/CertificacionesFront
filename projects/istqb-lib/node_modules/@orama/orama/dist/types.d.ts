...
export interface IIndex<I extends OpaqueIndex = OpaqueIndex> {
    create<S extends Schema, D extends OpaqueDocumentStore, So extends OpaqueSorter>(orama: Orama<{
        Schema: S;
        Index: I;
        DocumentStore: D;
        Sorter: So;
    ...
    insert: (implementation: IIndex<I>, index: I, prop: string, id: string, value: SearchableValue, schemaType: SearchableType, language: string | undefined, tokenizer: Tokenizer, docsCount: number) => SyncOrAsyncValue;
    afterInsert?: IIndexInsertOrRemoveHookFunction<I>;
    beforeRemove?: IIndexInsertOrRemoveHookFunction<I>;
    remove: (implementation: IIndex<I>, index: I, prop: string, id: string, value: SearchableValue, schemaType: SearchableType, language: string | undefined, tokenizer: Tokenizer, docsCount: number) => SyncOrAsyncValue<boolean>;
    afterRemove?: IIndexInsertOrRemoveHookFunction<I>;
    insertDocumentScoreParameters(index: I, prop: string, id: string, tokens: string[], docsCount: number): SyncOrAsyncValue;
    ...
    calculateResultScores<D extends OpaqueDocumentStore>(context: SearchContext<I, D>, index: I, prop: string, term: string, ids: string[]): SyncOrAsyncValue<TokenScore[]>;
    ...
    searchByWhereClause<D extends OpaqueDocumentStore>(context: SearchContext<I, D>, index: I, filters: Record<string, boolean | string | string[] | ComparisonOperator>): SyncOrAsyncValue<string[]>;
...
export interface IDocumentsStore<D extends OpaqueDocumentStore = OpaqueDocumentStore> {
    create<S extends Schema, I extends OpaqueIndex, So extends OpaqueSorter>(orama: Orama<{
        Schema: S;
        Index: I;
        DocumentStore: D;
        Sorter: So;
...
export interface ISorter<So extends OpaqueSorter = OpaqueSorter> {
    create<S extends Schema, I extends OpaqueIndex, D extends OpaqueDocumentStore>(orama: Orama<{
        Schema: S;
        Index: I;
        DocumentStore: D;
        Sorter: So;
    }>, schema: Schema, sorterConfig?: SorterConfig): SyncOrAsyncValue<So>;
    insert: (sorter: So, prop: string, id: string, value: SortValue, schemaType: SortType, language: string | undefined) => SyncOrAsyncValue;
...
type RequiredInner<T extends Partial<ProvidedTypes>> = {
    [Key in keyof ProvidedTypes]: Key extends keyof ProvidedTypes ? ProvidedTypes[Key] : Key extends keyof T ? T[Key] : never;
};
